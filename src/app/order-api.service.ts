...
export class OrderAPIService {
  ...
  public deleteitem(id:Number,oid:Number){
    this.http.delete("https://localhost:10649/OrderItems/"+id).subscribe((data) => 
    console.log(data));
    this.http.delete("https://localhost:10649/Orders/"+oid).subscribe((data) => 
    console.log(data));
    ;
...
/*
  ...
  public addAddress(add: Address,uid:number){
    add.userId=uid;
    return this.http.post("http://localhost:10649/Orders/UserAdd",add,{responseType:'text' as 'json'});
   }
   goToBill(){
    console.log("Bill");
 ...
 public getaddress(id:number){
  return this.http.get("https://localhost:44373/UserAddresses/"+id);
 }
